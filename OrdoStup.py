...
# Vérification pour ajouter (délivrance en une fois) si durée = rythme
    if patient_data["Rythme_de_Delivrance"] == patient_data["Duree"]:
        pdf.set_font("Arial", '', 10)
        pdf.cell(0, 10, f"A délivrer tous les {num2words(patient_data['Rythme_de_Delivrance'], lang='fr')} jours (délivrance en une fois)", ln=True, align="L")
    else:
        pdf.set_font("Arial", '', 10)
        pdf.cell(0, 10, f"A délivrer tous les {num2words(patient_data['Rythme_de_Delivrance'], lang='fr')} jours", ln=True, align="L")
